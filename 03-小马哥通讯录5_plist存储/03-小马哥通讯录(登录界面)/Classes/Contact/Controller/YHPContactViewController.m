...
#define YHPFilePath [NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES)[0] stringByAppendingString:@"contacts.data"]
...
- (NSMutableArray *)contacts
{
    if (_contacts == nil) {
        //  读取数据
        _contacts = [NSKeyedUnarchiver unarchiveObjectWithFile:YHPFilePath];
        if (_contacts == nil) {
...
//    传递联系人控制器
    ...
    addVc.blcok =^(YHPContact* conatct){
        // 1,联系人添加到数组
        [_contacts addObject:conatct];
        // 2，刷新表格
        [self.tableView reloadData];
        // 3,保存联系人
        [NSKeyedArchiver archiveRootObject:self.contacts toFile:YHPFilePath];
    };
}
...
{
    UITableViewRowAction* action = [UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleDefault title:@"增加" handler:^(UITableViewRowAction * _Nonnull action, NSIndexPath * _Nonnull indexPath) {
        // 点击增加的时候调用
        NSLog(@"增加");
    }];
    action.backgroundColor = [UIColor greenColor];
    UITableViewRowAction* action1 = [UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleDefault title:@"移除" handler:^(UITableViewRowAction * _Nonnull action, NSIndexPath * _Nonnull indexPath) {
        // 点击增加的时候调用
        NSLog(@"移除");
    }];
